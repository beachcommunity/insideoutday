...
function calculateMilestone() {
    ...
    const resultText = `A baby born on ${formattedBirthDate} at ${gestationalWeeks} weeks and ${gestationalDays} days gestation ` +
        `will have been alive for as long as they were in the womb on ${formattedMilestoneDate}.`;
...
function validateAndParseBirthDate(textInput, calendarInput) {
    let birthDate;
    if (textInput) {
        birthDate = dayjs(textInput, 'MM/DD/YYYY', true);
    } else if (calendarInput) {
        birthDate = dayjs(calendarInput);
    }
